...
class MyCasadiModel(CasadiModel):
    ...
    def setup_system(self):
        ...
        cp_air = 1000  # J/kg*K
        rho_air = 1.2  # kg/m³
        Vdot = [0.01, 0.01, 0.01, 0.01]  # Luftvolumenstrom m³/s zwischen den Zonen
        Q_air = [0]*4
        for i in range(4):
            i_next = (i + 1) % 4  # Index der nächsten Zone (Ring)
            i_prev = (i - 1) % 4  # Index der vorherigen Zone (Ring)
            Q_air[i] = (
                cp_air * rho_air * Vdot[i] * (T[i_next] - T[i])
                + cp_air * rho_air * Vdot[i_prev] * (T[i_prev] - T[i])
            )

        for i in range(4):
            T[i].ode = self.cp * mDot[i] / self.C * (T_in[i] - T[i]) + load[i] / self.C + Q_wall_in[i] / self.C + Q_air[i] / self.C
            T_out[i].alg = T[i]
...
AGENT_MPC = {
    ...
    "modules": [
        {"module_id": "Ag1Com", "type": "local_broadcast"},
        {
            ...
            "states": [
                *[{"name": f"T{i+1}", "value": 293.15 + i, "ub": 303.15, "lb": 288.15, "alias": f"T{i+1}", "source": "SimAgent"} for i in range(4)],
