...
from bes_rules.rule_extraction.innovization import run_brute_force_innovization
from studies.use_case_1_design import simulate_oed_cases, no_dhw

PATH_INPUT_ANALYSIS = RESULTS_FOLDER.joinpath("input_analysis", "dhw_optihorst")
PATH_OED = PATH_INPUT_ANALYSIS.joinpath("OED")
PATH_SIMULATIONS = RESULTS_FOLDER.joinpath("UseCase_TBivAndV", "dhw_optihorst")
FEATURES = [
    "Q_demand_total",
    "GTZ_Ti_HT",
    "THyd_nominal",
...
def perform_input_analysis():
    input_analysis.analyze_all(
        heat_pump=heat_pump_system.OptiHorst,
...
def run_simulations_from_no_dhw(oed_iter: int):
    ...
    simulate_oed_cases.run(
        ...
        model="MonoenergeticOptiHorst2DVCLibPy"
    )
...
def plot_SCOP_over_TBiv_for_cases(
    ...
    cases = {
        "BESMod": PATH_SIMULATIONS.joinpath("OED_iter_3", "DesignOptimizationResults"),
    ...
    no_dhw.plot_SCOP_over_TBiv_for_cases(
        ...
        save_path=PATH_SIMULATIONS.joinpath("compare_to_VDI"),
        cases=cases
    )
...
if __name__ == '__main__':
    ...
    #innovization_simulations(oed_iter=3, with_no_dhw_results=False)
