...
def convert_time_series_inputs_to_df(time_series_inputs: pd.DataFrame, time_step: float):
    ...
    df.index *= round(60 * 60 * time_step)
    first_day_in_year = datetime.datetime(2015, 1, 1)
...
def run_rbpc(simulation_settings, time_series_inputs, solver_kwargs, **kwargs):
    ...
    dTBufSet = cluster_map[y][0]
    if y > min(cluster_map.keys()):
        overheat = np.full(len(dTBufSet), True)     # if rbpc wants to overheat use external valve control (during sun hours)
    else:
        overheat = np.full(len(dTBufSet), False)    # if rbpc does not want to overheat use local valve control
    ...
    return {
        ...
        "actExtVal": np.full(len(dTBufSet), False),     # in modelica: if actExtVal use input all the time else just when overheating and sun
        "actOverheat": overheat,
        "yValSet": np.ones(len(dTBufSet)),    # optimization result: usually if mpc overheats valve is fully open | alt: cluster_map[y][1]
    }
