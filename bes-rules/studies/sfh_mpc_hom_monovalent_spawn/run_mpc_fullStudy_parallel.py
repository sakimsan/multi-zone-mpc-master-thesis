...
def run_mpc(
    ...
    #    return df
    if not isinstance(model_predictions_name, list):
        model_predictions_name = model_predictions_name
    if not isinstance(model_hom_name, list):
        model_hom_name = model_hom_name
    ...
    sim_config = base_design_optimization.get_simulation_config(
        model=model_predictions_name[0],
        model_hom=model_hom_name[0],
        # Anpassen!
    ...
    inputs_config = compare_buf_sizes.get_inputs_config(
        ...
        model_predictions_names=model_predictions_name,
        model_hom_names=model_hom_name
    )
    ...
    run_input_variations(
        config=config, run_inputs_in_parallel=True,
...
if __name__ == "__main__":
    ...
    run_mpc(
        model_predictions_name=model_predictions_names,
        model_hom_name=model_hom_names,
        study_name="studies_parallel",
        n_cpu=6,
