...
class MHE(BaseModule):
    ...
    def _callback_hist_vars(self, variable: AgentVariable, name: str):
        ...
        # Case distinction for the variable value
        if isinstance(variable.value, str) and (variable.value.startswith('{') and variable.value.endswith('}')):
            try:
                # Manual parsing of a simple dictionary string
                # Remove outer brackets and split by commas
                value_str = variable.value.strip('{}')
                pairs = [pair.strip() for pair in value_str.split(',')]
                ...
                for pair in pairs:
                    if ':' in pair:
                        ...
                        try:
                            ...
                            # If it's not a numeric value, keep the string
                            val = val.strip('"\'')

                        value_dict[key] = val

                # Use the value that corresponds to the timestamp
                timestamp_str = str(variable.timestamp)
                if timestamp_str in value_dict:
                    value_to_store = value_dict[timestamp_str]
                else:
                    # If no matching timestamp key was found, use the original value
                    value_to_store = variable.value
            except Exception:
                # In case of parsing errors, use the original value
                value_to_store = variable.value
        else:
            # For normal values, use the original value
            value_to_store = variable.value

        # Store the determined value in the history
        self.history.loc[variable.timestamp, name] = value_to_store
