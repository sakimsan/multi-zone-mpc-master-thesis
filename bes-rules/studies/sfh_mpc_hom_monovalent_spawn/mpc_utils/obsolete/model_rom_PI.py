...
class MPCModelConfig(CasadiModelConfig):
    ...
    states: List[CasadiState] = [
        ...
        CasadiState(name="yValState", value=0, unit="-",
                    description="set value of valve position"),
        # integral part of PI-controller
        CasadiState(name="PI_ctrl_integral", value=0, unit="K*s",
                    description="Integral term of PI controller"),
        ...
        CasadiState(name="yValState_slack", value=0, unit="-",
                    description="slack variable for yValState"),
    ...
    parameters: List[CasadiParameter] = [
        ...
        CasadiParameter(name="scale_obj", value=900, unit="K",
...
class MPC(CasadiModel):
    ...
    def setup_system(self):
        ...
        # calculate valve position and mass flow according to PI-controller
        valve_ctrl_k = 0.5 * bes_parameters["hydraulic.control.valCtrl.PI[1].k"]
        valve_ctrl_Ti = 2 * bes_parameters["hydraulic.control.valCtrl.PI[1].Ti"]
        error = self.TSetOneZone - self.T_Air
        #   DGL
        self.PI_ctrl_integral.ode = error
        pi_ctrl_output = valve_ctrl_k * error + (valve_ctrl_k / valve_ctrl_Ti) * self.PI_ctrl_integral   # pi_ctrl_output allowing all real values. Constraining the output would constrain the allowed temperature difference between TSetOneZone and TAir
        valve_output = pi_ctrl_output
        #valve_output = ca.fmin(ca.fmax(pi_ctrl_output, 0), 1)  # set limits [0,1]
        tau_valve = 900   # time constant in s
        #   DGL
        self.yValState.ode = (1 / tau_valve) * (valve_output - self.yValState)   # consider constraints of pi_ctrl [0,1]. If pi_ctrl_output < 0, yValState --> 0. If pi_ctrl_output > 1, yValState --> 1. Otherwise yValState --> pi_ctrl_output

        valve_actual = self.valve_leakage + self.yValState * (1 - self.valve_leakage)
        ...
        self.constraints = [
            ...
            # heatpump
            # todo: checken, ob wirklich minimal mit Tempertaur der Heizkurve
            (self.THeaCur, THeaPumSup + self.THeaPumSup_slack, 273.15 + 70),
            ...
            # valve
            (0.01, self.yValState + self.yValState_slack, 0.99),
            ...
            #(0, self.P_el_feed_into_grid, 10),
            #(0, self.P_el_feed_from_grid, 10000)
        ]
        ...
        C_el = self.c_grid / 3600 * PEleHeaPum / 1000  # €/kWh * 1h/3600s * W / 1000
        ...
        cost_terms = [
                ...
                10000 / self.scale_obj * self.TTraSup_slack ** 2,
                10000 / self.scale_obj * self.THeaPumSup_slack ** 2,
                10000 / self.scale_obj * self.QHeaPum_flow_slack ** 2,
                10000 / self.scale_obj * self.PEleHeaPum_slack ** 2,
                100000000 / self.scale_obj * self.yValState_slack ** 2,
                10000 / self.scale_obj * self.s_Pel_feed ** 2
            ]

        #for n in range(1, self.config.nLayer + 1):
        #    s_T_TES_n = self._states.get(f"s_T_TES_{n}")
        #    cost_terms.append(10000 / self.scale_obj * s_T_TES_n ** 2)
        ...
        self.P_el_demand.alg = PEleHeaPum
        self.P_el_demand_devices.alg = 0
        self.P_el_demand_lights.alg = 0

        self.P_pv.alg = self.P_el_pv
        self.P_el_feed.alg = PEleHeaPum
