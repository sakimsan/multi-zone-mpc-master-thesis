...
import numpy as np
import math
from bes_rules.rule_extraction.clustering import k_medoids
...
def cluster(time_series, number_clusters, n_days, norm=2, time_limit=900, mip_gap=0.0,
            weights=None):
    """
    ...
    """
    len_day = int(time_series.shape[1] / n_days)
    ...
    for i in range(time_series.shape[0]):
        ...
        inputsScaledTransformed.append(temp.reshape((len_day, n_days), order="F"))
        inputsTransformed.append(vals.reshape((len_day, n_days), order="F"))
    ...
    return scaled_typ_days, nc, z, inputsTransformed, obj
