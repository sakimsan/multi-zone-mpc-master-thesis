...
class SCOP(BaseObjective):
    ...
    def calc(self, df: pd.DataFrame, input_config: "InputConfig" = None):
        ...
        heating_rod_electricity_demand = self.get_values(df=df, name=self.mapping.heating_rod_electricity_demand)
        heating_rod_heat_supplied = self.get_values(df=df, name=self.mapping.heating_rod_heat_supplied)
