...
class BaseSupervisoryControl(SurrogateBuilder, abc.ABC):
    ...
    def generate_fmu(self, hom: bool = False):
        if hom:    # set 64 bit needed for spawn
            self.sim_api.dymola.ExecuteCommand("Advanced.CompileWith64=2;")
            self.sim_api.dymola.ExecuteCommand("Advanced.FMI.CompileFMU32=false;")
            self.sim_api.dymola.ExecuteCommand("Advanced.FMI.CompileFMU64=true;")
        # Create FMU
        self.sim_api.dymola.ExecuteCommand("Advanced.FMI.xmlIgnoreProtected = false;")
        res = self.sim_api.dymola.translateModelFMU(
            modelToOpen=self.sim_api.model_name if not hom else self.config.simulation.model_name_hom,
            storeResult=False,
            modelName="BES" if not hom else "BEShom",
            fmiVersion='2',
            fmiType='all' if not hom else 'cs',
            includeSource=False,
            includeImage=0
        )
    ...
    def simulate(self, parameters, log_df):
        ...
        if self.input_config.hom:
            fmu_path_hom = self.generate_fmu(hom=self.input_config.hom)

        skipped_results = []
        func_kwargs = []
        for idx, design_parameters in enumerate(parameters):
            path = self.get_log_path().parent.joinpath(f"Design_{idx}.xlsx")
            if not self.config.simulation.recalculate and os.path.exists(path):
                logger.info("Case %s already simulated, skipping...", path)
                skipped_results.append(path)
                continue
            # Else after continue
            ...
            if self.input_config.hom:
                bes_parameters_hom, state_result_names_hom, output_result_names_hom = get_fmu_model_parameters_and_split_names(
                    parameters=design_parameters,
                    fmu_path=fmu_path_hom,
                    result_names=self.get_result_names()
                )
                bes_parameters_hom.update(bes_parameters)
                bes_parameters = bes_parameters_hom
                state_result_names = state_result_names_hom
                output_result_names = output_result_names_hom
            ...
            func_kwargs.append(
                self.get_function_inputs_for_parameters(
                    ...
                    fmu_path=fmu_path if not self.input_config.hom else fmu_path_hom,
                    save_path=path,
        ...
        # Load results
        results = result_handling.merge_results(results=results, skipped_results=skipped_results)
