...
2) löst das lineare Oberflächen-Energiebilanzsystem (Konvektion + Leitung zum 1. RC-Knoten
   + linearisiert LW zwischen allen Innenflächen);
...
def solve_zone_surfaces_rc_only(
    surfaces: List[Dict],
    T_air: sp.Symbol,
    T_ref: float = 295.0,
    ...
    """
    Löst A*Ts = b für alle Innenoberflächen einer Zone.

    surfaces[i] erwartet:
      {
        'id': '<Instanz>',                # 'OuterWall', 'OuterWall2', 'Dach3', 'Decke', ...
        'type': '<Typ>',                  # 'OuterWall'|'InnerWall'|'Roof'|'GroundFloor'|'InnerFloor'|'Ceiling'
        'A': float,                       # Fläche [m²]
        'eps': float,                     # LW-Emissivität [-]
        'h_c': float,                     # Innen-Konvektion [W/m²K]
        'R_cond_areal': float,            # Areal-Widerstand [m²K/W] (Oberfläche -> erster RC-Knoten)
        'T_back': sympy.Symbol|float,     # erster RC-Knoten (zoneninterne State)
        'q_src': sympy.Expr|float=0.0     # Quellen [W] (flächen- & typgewichtet)
      }

    Rückgabe:
      coeffs: dict "T_s_<Instanz>" -> {"const":..., "<Symbol>": coeff, ...}
      Ts_keys: Liste der "T_s_<Instanz>" Keys
    """
    n = len(surfaces)
    ...
    # linearisiert LW via MRT-Elimination (H_ij = H_i*H_j / sum(H))
    h_r_i = [4.0*sigma*eps[i]*(T_ref**3) for i in range(n)]
    H_i   = [h_r_i[i]*A[i] for i in range(n)]
    H_tot = sum(H_i) if sum(H_i) > 0 else 1.0
    H_ij  = [[0.0]*n for _ in range(n)]
    for i in range(n):
        for j in range(n):
            if i != j:
                H_ij[i][j] = H_i[i]*H_i[j]/H_tot

    # System aufbauen
    A_mat = sp.zeros(n, n)
    b_vec = sp.zeros(n, 1)
    for i in range(n):
        diag = G_conv[i] + G_cond[i] + sum(H_ij[i][j] for j in range(n) if j!=i)
        A_mat[i,i] = sp.Float(diag)
        for j in range(n):
            if i != j:
                A_mat[i,j] = sp.Float(-H_ij[i][j])
        rhs = G_conv[i]*T_air + G_cond[i]*T_b[i] + q[i]
        b_vec[i,0] = rhs

    sol = sp.solve(A_mat*sp.Matrix(Ts_syms) - b_vec, Ts_syms, dict=True)[0]
...
def calc_resistances_zone_element_specific(
    ...
    extra = [Qtr, Qig, Qsol]
    coeffs, _ = solve_zone_surfaces_rc_only(
...
def default_params_rc_only() -> Dict:
    ...
    return {
        # DIN-Werte
        "h_c": {"GroundFloor": 0.7, "InnerFloor": 0.7, "OuterWall": 2.5, "InnerWall": 2.5, "Roof": 5, "Ceiling": 5},
        # TARP, Durchschnitt
        # "h_c": {"GroundFloor": 0.774, "InnerFloor": 0.825, "OuterWall": 1.443, "InnerWall": 1.188, "Roof": 1.899, "Ceiling": 1.423},
        # TARP, Min
        # "h_c": {"GroundFloor": 0.350, "InnerFloor": 0.525, "OuterWall": 1.096, "InnerWall": 0.437, "Roof": 1.457, "Ceiling": 0.549},
        # TARP, Max
        # "h_c": {"GroundFloor": 1.563, "InnerFloor": 1.235, "OuterWall": 1.586, "InnerWall": 1.356, "Roof": 2.136, "Ceiling": 1.613},
...
# params = default_params_rc_only()
# coeff_dict = calc_resistances_zone_element_specific("livingroom", material, zone_construction, params)
# apply_coeff_dict_to_model_outputs(model=m, zone="livingroom", coeff_dict=coeff_dict, verbose=True)
