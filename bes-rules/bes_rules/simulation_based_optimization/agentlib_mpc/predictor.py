...
from ebcpy.preprocessing import build_average_on_duplicate_rows
...
class PredictorModule(al.BaseModule):
    ...
    def __init__(self, *, config: dict, agent: Agent):
        ...
        self.disturbances = build_average_on_duplicate_rows(self.disturbances)

        pd.read_csv(self.get("disturbances_path").value, index_col=0, dtype=float).to_csv("disturbances_raw.csv")
        #self.disturbances.to_csv("disturbances_interpolated.csv")
