...
def calculate_operating_costs_with_dynamic_prices(study_config: "StudyConfig", year):
    ...
    for df, input_config in zip(dfs, input_configs):
        ...
        for idx, row in df.iterrows():
            ...
            tsd.loc[:, "dynamic_costs_operation"] = (
                    tsd.loc[:, "outputs.electrical.dis.PEleLoa.value"] / 1000 * c_grid / 3600 * time_step
            )
            dynamic_costs.append(tsd.loc[:, "dynamic_costs_operation"].cumsum().iloc[-1])
        df.loc[:, f"dynamic_costs_operation_{year}"] = dynamic_costs
