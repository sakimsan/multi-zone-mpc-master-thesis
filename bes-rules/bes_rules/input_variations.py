...
def run_single_inputs_combination(
    ...
    if input_config.user.custom_model_hom is not None:
        config.simulation.model_name_hom = input_config.user.custom_model_hom
    if not use_mp:
...
def run_input_variations(
    ...
    model_names = [
        config.simulation.get_modified_model_name(input_config.get_modelica_modifier(with_custom_modifier=kwargs.get('with_custom_modifiers', True)))
